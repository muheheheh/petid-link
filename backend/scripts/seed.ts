...
// 设备1 - 已绑定用户+已绑定宠物1
db.run(
  `INSERT INTO devices (id, user_id, pet_id, bound_at, created_at) VALUES (?, ?, ?, ?, ?)`,
  [deviceId1, userId, petId1, now - 3600000, now - 86400000 * 7]
);

// 设备2 - 已绑定用户但未绑定宠物
db.run(
  `INSERT INTO devices (id, user_id, pet_id, bound_at, created_at) VALUES (?, ?, ?, ?, ?)`,
  [deviceId2, userId, null, now - 3600000, now - 86400000 * 3]
);

console.log("✅ 假数据插入完成");
console.log(`  用户: ${userId} (张小明)`);
console.log(`  宠物: ${petId1} (豆豆 - 金毛), ${petId2} (咪咪 - 英短)`);
console.log(`  设备: ${deviceId1} (已绑定豆豆), ${deviceId2} (已绑定用户未绑定宠物)`);
