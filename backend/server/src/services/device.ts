...
/** 激活设备：校验激活码，绑定到用户 */
export function activateDevice(deviceId: string, activationCode: string, userId: string) {
  const device = db.select().from(devices).where(eq(devices.id, deviceId)).get();
  if (!device) throw new BizError(ERR.DEVICE_NOT_FOUND);
  if (device.userId) throw new BizError(ERR.DEVICE_ALREADY_ACTIVATED);
  if (device.activationCode !== activationCode) throw new BizError(ERR.DEVICE_CODE_INVALID);

  db.update(devices)
    .set({ userId, activatedAt: new Date() })
...
export function bindPetToDevice(params: BindPetParams): string {
  ...
  if (!device) throw new BizError(ERR.DEVICE_NOT_FOUND);
  if (!device.userId) throw new BizError(ERR.DEVICE_NOT_ACTIVATED);
...
export function getDevicePublicInfo(deviceId: string) {
  ...
  if (!device.userId) return { activated: false, pet: null };
  if (!device.petId) return { activated: true, pet: null };

  const pet = db.select().from(pets).where(eq(pets.id, device.petId)).get();
  if (!pet) return { activated: true, pet: null };
