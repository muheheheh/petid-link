...
export async function verifyAdminCredentials(username: string, password: string) {
  ...
  if (!admin) throw new BizError(ERR.AUTH_LOGIN_FAILED);

  const valid = await Bun.password.verify(password, admin.passwordHash);
  if (!valid) throw new BizError(ERR.AUTH_LOGIN_FAILED);
...
export function getDeviceList(params: PaginationParams & { userId?: string; status?: string[]; batch?: string }) {
  ...
  if (params.status?.length) {
    ...
    for (const s of params.status) {
      if (s === "unbound") statusConds.push(sql`${devices.userId} IS NULL`);
      if (s === "bound") statusConds.push(and(sql`${devices.userId} IS NOT NULL`, sql`${devices.petId} IS NULL`));
      if (s === "bindPet") statusConds.push(sql`${devices.petId} IS NOT NULL`);
    }
...
export function batchCreateDevices(count: number, batch?: string): { ids: string[]; batch: string } {
  ...
  for (let i = 0; i < count; i++) {
    const id = crypto.randomUUID();
    db.insert(devices).values({ id, batch: batchNo }).run();
    ids.push(id);
  }
...
/** 管理后台解绑设备（释放设备归属） */
export function adminUnbindDevice(deviceId: string) {
  const device = db.select().from(devices).where(eq(devices.id, deviceId)).get();
  if (!device) throw new BizError(ERR.COMMON_NOT_FOUND);
...
export function getDashboardStats(startTime?: number, endTime?: number) {
  ...
  const deviceBoundUserCount = db.select({ count: sql<number>`count(*)` }).from(devices).where(sql`${devices.userId} IS NOT NULL`).get()?.count ?? 0;
  const deviceBoundPetCount = db.select({ count: sql<number>`count(*)` }).from(devices).where(sql`${devices.petId} IS NOT NULL`).get()?.count ?? 0;
  ...
  return {
    ...
    device_bound_user_count: deviceBoundUserCount,
    device_bound_pet_count: deviceBoundPetCount,
